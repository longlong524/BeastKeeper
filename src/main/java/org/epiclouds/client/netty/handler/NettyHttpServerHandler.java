...
public class NettyHttpServerHandler extends ChannelHandlerAdapter{
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg)
		...
		if(res.method().compareTo(HttpMethod.CONNECT)==0){
			...
			ctx.close();
			//System.err.println("Connect close channel:"+count.incrementAndGet()+" channel"+ctx.channel());
			return;
		}
		...
		if((res.headers().get("Host")+"").startsWith("106.3.38.50")
			...
			ReferenceCountUtil.release(msg);
			ctx.close();
			System.err.println("Host Local close channel:"+count.incrementAndGet()+" channel"+ctx.channel());
			return;
		}
