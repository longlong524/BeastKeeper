						...
						function getSyncInstance(name){
							$.ajax({
								  ...
								  success: function(data,statustxt){
									  syncInstance=$.evalJSON(data);
									  if(syncInstance.info!="ok"){
										  alert("错误信息:"+syncInstance.info);
										  return;
									  }
									  $("#name").val(syncInstance.name);
									  $("#start").val(syncInstance.start);
									  $("#start").change(function(){
										  syncInstance.start=$("#start").val();
									  });
									  $("#conip").val(syncInstance.remoteip);
									  $("#conport").val(syncInstance.port);
									  $("#conip").change(function(){
										  syncInstance.remoteip=$("#conip").val();
									  });
									  $("#conport").change(function(){
										  syncInstance.port=$("#conport").val();
									  });
									  getSourceType(function(){
										  $("#src_type").val(syncInstance.sourceconfig.name);
										  getSourceConfigure();
										  for(var i in syncInstance.sourceconfig.configs){
											  $("#"+i).val(syncInstance.sourceconfig.configs[i]);
										  }
									  });
									  $("#source_handler_chain").val($.toJSON(syncInstance.sourceconfig.handlers));
									  $("#local_handler_chain2").val($.toJSON(syncInstance.localhandlers));
								  }
							});
						};
						...
						function addSourceHandlerChain(){
							...
							for(var i=0;i<hand.config.length;i++){
								hand.config[i].value=$("div#source div #"+hand.config[i].name).val();
							}
							  ...
							  for(var j=1;j<=3;j++){
								  ...
								  for(var i=k;i<k+divs[j-1];i++){
									 ...
									 inp.change(function(inp,i){
										 return function(){
											 syncInstance.sourceconfig.configs[data[i].name]=inp.val();
										 }
									 }(inp,i));
									  if(data[i].datatype!="no"&&data[i].datatype!=undefined){
